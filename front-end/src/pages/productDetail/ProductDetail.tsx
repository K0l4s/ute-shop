...
const ProductDetail: React.FC = () => {
  ...
  const userAddress = useSelector(
    (state: RootState) =>
      ...
      ', ' +
      state.auth.user?.ward +
      ', ' +
      state.auth.user?.district +
      ', ' +
    ...
    <div className='font-sans pb-8'>
      <main className='container mx-auto mt-8'>
            ...
            <div className='space-y-4'>
              <div className='relative'>
                ...
                <div className='relative group'>
                  ...
                  <span
                    className='tooltip opacity-0 group-hover:opacity-100 absolute -top-8 -right-44
              ...
              <p className='font-semibold text-green-600'>
                Tình trạng: {book.stock > book.sold_count + 1 ? 'Còn hàng' : 'Hết hàng'}
              </p>
              ...
              <div className='flex items-center space-x-2'>
                <label className='text-2xl flex'>{formatStar(averageRating)}</label>
                <span>({reviewList.length} đánh giá)</span>
              </div>
              {book.stock > book.sold_count + 1 ? (
                <div className='cursor-pointer text-base'>
                  <button
                    className={`${book.stock > book.sold_count + 1 ? 'border-violet-700 border-2' : 'bg-gray-200'}  w-44 font-semibold text-violet-700 px-4 py-2 rounded-lg hover:bg-violet-700 hover:text-white transition-colors`}
                ...
                <button className='w-44 bg-gray-300 font-semibold text-black px-4 py-2 rounded-lg mr-5 cursor-not-allowed'>
                  Hết hàng
                </button>
              )}
            </div>
              ...
              {isAuthenticated && (
                <>
                  ...
                  <div className='flex items-center gap-8 mt-2'>
                    ...
                    <Link to='/account/address' className='font-semibold text-violet-600 hover:text-violet-700'>
                      Thay đổi
                    </Link>
                  </div>
                </>
        ...
        <div className='section bg-white rounded-xl p-10 mt-8 text-lg'>
          ...
          <pre className='mt-4 whitespace-pre-wrap' style={{ fontFamily: 'inherit', fontSize: 'inherit' }}>
            {book.desc}
          </pre>
        </div>
              ...
              {filteredReviews.map((review, index) => (
                <div key={index} className='bg-gray-100 rounded-lg p-4'>
                  <p className='font-bold'>
                    {review.User ? review.User.lastName + ' ' + review.User.firstName : 'Khách truy cập'}
                  </p>
