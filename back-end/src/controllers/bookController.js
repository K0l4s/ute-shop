...
const createBookController = async (req, res) => {
  ...
  const { ISBN, title, desc, price, salePrice, year, age, stock, author_id, publisher_id } = req.body;
  ...
  try {
    ...
    const book = await createNewBook({ISBN, title, desc, price, salePrice, year, age, stock,cover_img_url, author_id, publisher_id});
...
const getBooksByListIdController = async (req, res) => {
  const { ids } = req.body;

  try {
    const books = await getBooksByListId(ids);
    return res.status(200).json({
      message: "success",
      data: books
    })
